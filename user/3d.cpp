...
int d3d::CNFGPenX, d3d::CNFGPenY;
 void (*CNFGTackPixel)( int x, int y ); //Unsafe plot pixel.
uint8_t * d3d::frontframe;
int16_t d3d::ModelviewMatrix[16];
int16_t d3d::ProjectionMatrix[16];
uint8_t d3d::CNFGBGColor;
uint8_t d3d::CNFGLastColor;
uint16_t LTW = FBW;
uint8_t d3d::CNFGDialogColor; //background for boxes
d3d::d3d(){}
...
int16_t d3d::tdCOS( uint8_t iv )
{
	return d3d::tdSIN( iv + 64 );
}


void ICACHE_FLASH_ATTR d3d::MakeXRotationMatrix( uint8_t angle, int16_t * f )
{
	f[0] = 256;  f[1] = 0;  f[2] = 0;  f[3] = 0;
	f[4] = 0;  f[5] = d3d::tdCOS( angle );  f[6] = - d3d::tdSIN( angle );  f[7] = 0;
	f[8] = 0;  f[9] = d3d::tdSIN( angle );  f[10] = d3d::tdCOS( angle );  f[11] = 0;
	f[12] = 0;  f[13] = 0;  f[14] = 0;  f[15] = 256;
}

void ICACHE_FLASH_ATTR d3d::MakeYRotationMatrix( uint8_t angle, int16_t * f )
{
	f[0] = d3d::tdCOS( angle );  f[1] = 0;  f[2] = d3d::tdSIN( angle );  f[3] = 0;
	f[4] = 0;  f[5] = 256;  f[6] = 0;  f[7] = 0;
	f[8] = -d3d::tdSIN( angle );  f[9] = 0;  f[10] = d3d::tdCOS( angle );  f[11] = 0;
	f[12] = 0;  f[13] = 0;  f[14] = 0;  f[15] = 256;
}

void ICACHE_FLASH_ATTR d3d::tdIdentity( int16_t * matrix )
{
...
void ICACHE_FLASH_ATTR d3d::Perspective( int fovx, int aspect, int zNear, int zFar, int16_t * out )
{
	int16_t f = fovx;
...
void ICACHE_FLASH_ATTR d3d::MakeTranslate( int x, int y, int z, int16_t * out )
{
	d3d::tdIdentity(out);
	out[m03] += x;
	out[m13] += y;
...
void ICACHE_FLASH_ATTR d3d::tdTranslate( int16_t * f, int16_t x, int16_t y, int16_t z )
{
	int16_t ftmp[16];
	d3d::tdIdentity(ftmp);
	ftmp[m03] += x;
	ftmp[m13] += y;
	ftmp[m23] += z;
	d3d::tdMultiply( f, ftmp, f );
}

void ICACHE_FLASH_ATTR d3d::tdScale( int16_t * f, int16_t x, int16_t y, int16_t z )
{
	f[m00] = (f[m00] * x)>>8;
	f[m01] = (f[m01] * x)>>8;
	f[m02] = (f[m02] * x)>>8;
...
void ICACHE_FLASH_ATTR d3d::tdRotateEA( int16_t * f, int16_t x, int16_t y, int16_t z )
{
	int16_t ftmp[16];

	//x,y,z must be negated for some reason
	int16_t cx = d3d::tdCOS(x);
	int16_t sx = d3d::tdSIN(x);
	int16_t cy = d3d::tdCOS(y);
	int16_t sy = d3d::tdSIN(y);
	int16_t cz = d3d::tdCOS(z);
	int16_t sz = d3d::tdSIN(z);
...
void ICACHE_FLASH_ATTR d3d::tdMultiply( int16_t * fin1, int16_t * fin2, int16_t * fout )
{
	int16_t fotmp[16];
...
void ICACHE_FLASH_ATTR d3d::tdPTransform( int16_t * pin, int16_t * f, int16_t * pout )
{
	int16_t ptmp[2];
...
void d3d::td4Transform( int16_t * pin, int16_t * f, int16_t * pout )
{
	int16_t ptmp[3];
...
void ICACHE_FLASH_ATTR d3d::LocalToScreenspace( int16_t * coords_3v, int16_t * o1, int16_t * o2 )
{
	int16_t tmppt[4] = { coords_3v[0], coords_3v[1], coords_3v[2], 256 };
	d3d::td4Transform( tmppt, d3d::ModelviewMatrix, tmppt );
	d3d::td4Transform( tmppt, d3d::ProjectionMatrix, tmppt );
	if( tmppt[3] >= 0 ) { *o1 = -1; *o2 = -1; return; }

	if( d3d::CNFGLastColor > 15 )
	{
		//Half-height mode
...
{
	d3d::frontframe[(x+y*FBW)>>2] |=   2<<( (x&3)<<1 );
}
...
{
	uint8_t * ffs = &d3d::frontframe[(x+y*FBW2)>>1];
	if( x & 1 )
	{
		*ffs = (*ffs & 0x0f) | d3d::CNFGLastColor<<4;
	}
	else
	{
		*ffs = (*ffs & 0xf0 ) | d3d::CNFGLastColor;
	}
}

void d3d::CNFGColor( uint8_t col )
{
	d3d::CNFGLastColor = col;
	if( col == 16 )
	{
		LTW = FBW;
...
void d3d::CNFGTackSegment( int x0, int y0, int x1, int y1 )
{
	int deltax = x1 - x0;
	int deltay = y1 - y0;
	int error = 0;
	int x;
	int sy = d3d::LABS(deltay);
	int ysg = (y0>y1)?-1:1;
	int y = y0;
	...
	if( d3d::CNFGLastColor  )
	{
		...
		int deltaerr = d3d::LABS((deltay * 256) / deltax);
		int xsg = (x0>x1)?-1:1;
...
void ICACHE_FLASH_ATTR d3d::Draw3DSegment( int16_t * c1, int16_t * c2 )
{
	int16_t sx0, sy0, sx1, sy1;
	d3d::LocalToScreenspace( c1, &sx0, &sy0 );
	d3d::LocalToScreenspace( c2, &sx1, &sy1 );
	d3d::CNFGTackSegment( sx0, sy0, sx1, sy1 );
}

void ICACHE_FLASH_ATTR d3d::DrawGeoSphere()
{
	int i;
...
void ICACHE_FLASH_ATTR d3d::CNFGDrawText( const char * text, int scale )
{
	...
	while( text[place] )
	{
		...
		default:
			...
			do
			{
				...
				lmap++;
				d3d::CNFGTackSegment( x1, y1, x2, y2 );
				bQuit = *lmap & 0x80;
				lmap++;
			} while( !bQuit );
...
void ICACHE_FLASH_ATTR d3d::CNFGDrawBox(  int x1, int y1, int x2, int y2 )
{
	unsigned lc = d3d::CNFGLastColor;
	d3d::CNFGColor( d3d::CNFGDialogColor );
	d3d::CNFGTackRectangle( x1, y1, x2, y2 );
	d3d::CNFGColor( lc );
	d3d::CNFGTackSegment( x1, y1, x2, y1 );
	d3d::CNFGTackSegment( x2, y1, x2, y2 );
	d3d::CNFGTackSegment( x2, y2, x1, y2 );
	d3d::CNFGTackSegment( x1, y2, x1, y1 );
}

void ICACHE_FLASH_ATTR d3d::CNFGTackRectangle( short x1, short y1, short x2, short y2 )
{
	short ly = 0;
	short my = 0;
	short lx = 0;
...
int16_t ICACHE_FLASH_ATTR d3d::tdNoiseAt( int16_t x, int16_t y )
{
...
int16_t ICACHE_FLASH_ATTR d3d::tdFLerp( int16_t a, int16_t b, int16_t t )
{
	int16_t fr = tdFade( t );
...
static inline int16_t ICACHE_FLASH_ATTR tdFNoiseAt( int16_t x, int16_t y )
{
   // d3d dd();
	...
	int16_t a = d3d::tdNoiseAt( ix, iy );
	int16_t b = d3d::tdNoiseAt( ix+1, iy );
	int16_t c = d3d::tdNoiseAt( ix, iy+1 );
	int16_t d = d3d::tdNoiseAt( ix+1, iy+1 );

	int16_t top = d3d::tdFLerp( a, b, fx );
	int16_t bottom = d3d::tdFLerp( c, d, fx );

	return d3d::tdFLerp( top, bottom, fy );
}

int16_t ICACHE_FLASH_ATTR d3d::tdPerlin2D( int16_t x, int16_t y )
{
	int ndepth = 5;
