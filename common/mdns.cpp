...
void ICACHE_FLASH_ATTR mdns::SetupMDNS() { }
int ICACHE_FLASH_ATTR mdns::JoinGropMDNS() { }
void ICACHE_FLASH_ATTR mdns::AddMDNSService( const char * ServiceName, const char * Text, int port ) { }
void ICACHE_FLASH_ATTR mdns::AddMDNSName( const char * ToDup ) { }
...
void ICACHE_FLASH_ATTR mdns::AddMDNSService( const char * ServiceName, const char * Text, int port )
{
	...
	for( i = 0; i < MAX_MDNS_SERVICES; i++ )
	{
		if( !MDNSServices[i] )
		{
			MDNSServices[i] = mystuff::strdupcaselower( ServiceName );
...
void ICACHE_FLASH_ATTR mdns::AddMDNSAddName( const char * ToDup )
{
	int i;
	for( i = 1; i < MAX_MDNS_NAMES; i++ )
	{
		if( !MDNSNames[i] )
		{
			...
			MDNSNames[i] = mystuff::strdupcaselower( ToDup );
			break;
		}
	}
}

void ICACHE_FLASH_ATTR mdns::ClearMDNSNames()
{
	int i;
	...
	MDNSNames[0] = mystuff::strdupcaselower( &CommonSettings::SETTINGS.DeviceName[0] );
}

uint8_t * ICACHE_FLASH_ATTR mdns::ParseMDNSPath( uint8_t * dat, char * topop, int * len )
{
	int l;
	int j;
	*len = 0;
...
uint8_t * ICACHE_FLASH_ATTR mdns::SendPathSegment( uint8_t * dat, const char * path )
{
	char c;
	int i;
	do
...
uint8_t * ICACHE_FLASH_ATTR mdns::TackTemp( uint8_t * obptr )
{
	...
	*(obb++) = 0;
	...
	for( i = 0; i < nr_services; i++ )
	{
		...
		*(obptr++) = 0x00; *(obptr++) = sl+8;
		obptr = mdns::SendPathSegment( obptr, localservicename );
		*(obptr++) = 0x00;
	}
...
//	ets_memcpy( obptr, namestartptr, stlen+1 ); //Hack: Copy the name in.
//	obptr += stlen+1;
	obptr = mdns::SendPathSegment( obptr,  sn );
	obptr = mdns::SendPathSegment( obptr,  "local" );
	*(obptr++) = 0;
	...
	*(obptr++) = stl; //Service length
	...
	if( sp )
	{
		...
		*(obptr++) = sp>>8; *(obptr++) = sp&0xff; //Port#
		obptr = mdns::SendPathSegment( obptr, MyLocalName );
		*(obptr++) = 0;
	}
...
static void ICACHE_FLASH_ATTR got_mdns_packet(void *arg, char *pusrdata, unsigned short len)
{
	...
	else
	{
		...
		for( i = 0; i < questions; i++ )
		{
			...
			dataptr = mdns::ParseMDNSPath( dataptr, path, &stlen );
...
int ICACHE_FLASH_ATTR mdns::JoinGropMDNS()
{
	uint32_t ip = mystuff::GetCurrentIP();
	if( ip )
	{
		/*
	//enum lwip_ip_addr_type ty = lwip_ip_addr_type.IPADDR_TYPE_V4;
		struct ip_addr_t grpip = sizeof(ip4_addr_t)  ;
		grpip.addr = MDNS_BRD;
